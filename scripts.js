...
const hollowBtn = document.getElementById('hollow');
...
hollowBtn.addEventListener('click', () => {
   // Toggle between the light and dark themes
   document.body.classList.toggle('hollow');
});
...
if (uname === "say_german") {
   ...
   if (cookedMatch) {
      ...
      idk = true;
      text = idkMatch[1];
   } else if (glowinggoldMatch) {
      glowinggold = true;
      text = glowinggoldMatch[1];
   } else if (slimeMatch) {
      slime = true;
      text = slimeMatch[1];
   } else if (axeyMatch) {
      ...
      special = true;
      text = specialMatch[1];
   } else if (pokemonMatch) {
      pokemon = true;
      text = pokemonMatch[1];
   } else if (minecraftMatch) {
      minecraft = true;
      text = minecraftMatch[1];
   } else if (glitchMatch) {
      glitch = true;
      text = glitchMatch[1];
   } else if (frozenMatch) {
      frozen = true;
      text = frozenMatch[1];
   } else if (slimenewMatch) {
      slimenew = true;
      text = slimenewMatch[1];
   } else if (silksongMatch) {
      silksong = true;
      text = silksongMatch[1];
   } else if (oiiaMatch) {
      oiia = true;
      text = oiiaMatch[1];
   } else if (maxwellMatch) {
      maxwell = true;
      text = maxwellMatch[1];
   } else if (undertaleMatch) {
      undertale = true;
      text = undertaleMatch[1];
   } else if (hexMatch) {
      color = `#${hexMatch[1]}`;
      text = hexMatch[2];
   }
...
// Clear new messages count and title when user revisits the website
window.addEventListener('focus', () => {
   newMessagesCount = 0;
   document.title = 'Logans Tools For School';
});
